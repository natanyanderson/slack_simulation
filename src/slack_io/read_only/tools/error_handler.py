...
ERROR_MESSAGES = {
    ...
    "missing_scope": "I don't have the required permissions for that action. The search.messages API requires a user token, not a bot token. This is a Slack API limitation. You can try searching specific channels instead using get_channel_history.",
    "not_allowed_token_type": "The search.messages API requires a user token (xoxp-), but I'm using a bot token (xoxb-). This is a Slack API limitation. As an alternative, I can search specific channels for you. Try asking me to search a specific channel like 'Search #channel-name for payment gateway'.",
